    ...
    public partial class frmManageParameters : Form
    {
        ...
        private void LoadDetails()
        {
            try
            {
                ...
                GridFunc.PopulateDataGrid(dgParameters, dtCloned);
                // set grid propoerties
                GridFunc.SetGridProperties(this.Name, dgParameters);
            }
            catch (Exception ex)
            {
                commonLib.showUIMessage(ex.Message, MessageBoxButtons.OK, MessageBoxIcon.Stop);
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                for (int i = 0; i < dgParameters.Rows.Count; i++)
                {
                    string strCode;
                    string strName;
                    string strValue;
                    int iActive;
                    strCode = dgParameters.Rows[i].Cells[0].Value.ToString();
                    strName = dgParameters.Rows[i].Cells[1].Value.ToString();
                    strValue = dgParameters.Rows[i].Cells[2].Value.ToString();
                    iActive = Convert.ToInt32(dgParameters.Rows[i].Cells[3].Value);
